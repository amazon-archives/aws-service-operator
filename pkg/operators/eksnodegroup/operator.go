...
func NewOperator(config config.Config, queueManager *queuemanager.QueueManager) *Operator {
	...
	topicARN, _ := queuectrl.Register("eksnodegroup")
...
func (c *Operator) StartWatch(ctx context.Context, namespace string) {
	...
	oper := operator.New("eksnodegroups", namespace, resourceHandlers, c.config.AWSClientset.RESTClient())
	oper.Watch(&awsV1alpha1.EKSNodeGroup{}, ctx.Done())
}
...
func QueueUpdater(config config.Config, msg *queuemanager.MessageBody) error {
	...
	if msg.Updatable {
		...
		resources, err := clientSet.EKSNodeGroups("").List(metav1.ListOptions{})
		if err != nil {
			logger.WithError(err).Error("error getting eksnodegroups")
			return err
		}
...
func (c *Operator) onAdd(obj interface{}) {
	...
	if s.Status.ResourceStatus == "" || s.Status.ResourceStatus == "DELETE_COMPLETE" {
		...
		if err != nil {
			c.config.Logger.WithError(err).Errorf("error creating eksnodegroup '%s'", s.Name)
			return
		}
		c.config.Logger.Infof("added eksnodegroup '%s' with stackID '%s'", s.Name, string(*output.StackId))
...
func (c *Operator) onUpdate(oldObj, newObj interface{}) {
	oo := oldObj.(*awsV1alpha1.EKSNodeGroup).DeepCopy()
	no := newObj.(*awsV1alpha1.EKSNodeGroup).DeepCopy()
	...
	if helpers.IsStackComplete(oo.Status.ResourceStatus, false) && !reflect.DeepEqual(oo.Spec, no.Spec) {
		...
		if err != nil {
			c.config.Logger.WithError(err).Errorf("error updating eksnodegroup '%s' with new params %+v and old %+v", no.Name, no, oo)
			return
		}
		c.config.Logger.Infof("updated eksnodegroup '%s' with params '%s'", no.Name, string(*output.StackId))
		c.config.Logger.Infof("view at https://console.aws.amazon.com/cloudformation/home?#/stack/detail?stackId=%s", string(*output.StackId))
...
func (c *Operator) onDelete(obj interface{}) {
	...
	err := cft.DeleteStack()
	if err != nil {
		c.config.Logger.WithError(err).Errorf("error deleting eksnodegroup '%s'", s.Name)
		return
	}

	c.config.Logger.Infof("deleted eksnodegroup '%s'", s.Name)
}
func incrementRollbackCount(config config.Config, name string, namespace string) error {
	...
	_, err = clientSet.EKSNodeGroups(namespace).Update(resourceCopy)
	if err != nil {
...
func updateStatus(config config.Config, name string, namespace string, stackID string, status string, reason string) (*awsV1alpha1.EKSNodeGroup, error) {
	logger := config.Logger
	clientSet, _ := awsclient.NewForConfig(config.RESTConfig)
	resource, err := clientSet.EKSNodeGroups(namespace).Get(name, metav1.GetOptions{})
	if err != nil {
	...
	if helpers.IsStackComplete(status, false) {
		...
		resourceCopy.Output.NodeInstanceRole = outputs["NodeInstanceRole"]
		resourceCopy.Output.NodeSecurityGroup = outputs["NodeSecurityGroup"]
	}
...
func deleteStack(config config.Config, name string, namespace string, stackID string) (*awsV1alpha1.EKSNodeGroup, error) {
	logger := config.Logger
	clientSet, _ := awsclient.NewForConfig(config.RESTConfig)
	resource, err := clientSet.EKSNodeGroups(namespace).Get(name, metav1.GetOptions{})
	if err != nil {
...
func syncAdditionalResources(config config.Config, s *awsV1alpha1.EKSNodeGroup) (err error) {
	clientSet, _ := awsclient.NewForConfig(config.RESTConfig)
	resource, err := clientSet.EKSNodeGroups(s.Namespace).Get(s.Name, metav1.GetOptions{})
	if err != nil {
		return err
	}
	...
	configmaps := []string{}
	eksNodeGroupCMData := map[string]string{
		"nodeInstanceRole":  "{{.Obj.Output.NodeInstanceRole}}",
		"nodeSecurityGroup": "{{.Obj.Output.NodeSecurityGroup}}",
	}
	eksNodeGroupCM := helpers.CreateConfigMap(config, s, s.Name, s.Namespace, eksNodeGroupCMData)
	configmaps = append(configmaps, eksNodeGroupCM)
	resource.AdditionalResources.ConfigMaps = configmaps

	_, err = clientSet.EKSNodeGroups(s.Namespace).Update(resource)
	if err != nil {
		return err
	}
	return nil
