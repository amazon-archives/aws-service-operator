...
func (s *Cloudformation) CreateStack() (output *cloudformation.CreateStackOutput, err error) {
	...
	cftemplateURL := helpers.GetCloudFormationTemplate(s.config, "ecrrepository", s.ECRRepository.Spec.CloudFormationTemplateName, s.ECRRepository.Spec.CloudFormationTemplateNamespace)

	cftemplate, err := helpers.FetchAndProcessTemplate(cftemplateURL, s.ECRRepository)
	if err != nil {
		return output, err
	}
