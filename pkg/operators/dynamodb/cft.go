...
func (s *Cloudformation) CreateStack() (output *cloudformation.CreateStackOutput, err error) {
	...
	cftemplateURL := helpers.GetCloudFormationTemplate(s.config, "dynamodb", s.DynamoDB.Spec.CloudFormationTemplateName, s.DynamoDB.Spec.CloudFormationTemplateNamespace)

	cftemplate, err := helpers.FetchAndProcessTemplate(cftemplateURL, s.DynamoDB)
	if err != nil {
		return output, err
	}
